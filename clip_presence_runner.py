...
def run_single(dataset_path, balance_strategy, captions_file, run_id, seed,
    ...
    from lora_vision_multimodal import (VisionLoRAModel, TextOnlyLoRAModel, BothLoRAModel)
    import open_clip
    ...
    config = create_config(dataset_path=dataset_path, output_dir="/tmp/clippresence",
    ...
    logger.info("B) text-only (caption) LoRA")
    out['text_only_lora'] = train_one(TextOnlyLoRAModel(device=device).to(device))

    logger.info("C) vision-only (image) LoRA")
    out['image_only_lora'] = train_one(VisionLoRAModel(device=device).to(device))

    logger.info("D) both multimodal LoRA")
    out['both_lora'] = train_one(BothLoRAModel(device=device).to(device))

    def g(k): return out[k]['test_metrics']['mcc']
    logger.info(f"  MCC frozen={g('multimodal_frozen'):.3f} txt={g('text_only_lora'):.3f} "
...
def compute_stats(succ):
    ...
    out['condition_means'] = {c: {m: float(np.nanmean(col(c, m))) for m in metrics}
                              for c in ['multimodal_frozen', 'text_only_lora',
                                        'image_only_lora', 'both_lora']}
    return out
...
def run_experiment(dataset_path, balance_strategy, captions_file, output_dir, split_mode, ds_tag):
    ...
    logger = setup_logging("INFO", os.path.join(output_dir, f'clippresence_{split_mode}_{ds_tag}_log.txt'))
    logger.info("=" * 70)
    logger.info(f"CLIP PRESENCE 4-WAY | split={split_mode} ds={ds_tag}")
    logger.info("A frozen | B text-only | C image-only | D both-LoRA")
    logger.info("=" * 70)
...
def _dump(output_dir, split_mode, ds_tag, succ, failed, balance_strategy):
    payload = {'experiment_type': f'clip_presence_{split_mode}', 'ds_tag': ds_tag,
    ...
    fn = os.path.join(output_dir, f"clip_presence_{split_mode}_{ds_tag}_{balance_strategy}_final.json")
