...
class IdentityCaptionGenerator:
    ...
    def __init__(self, device: str = "auto", logger=None):
        ...
        self.logger.info("Loading BLIP2 model (identity-attribute variant)...")
        try:
    ...
    def generate_captions_for_image(self, image_path: str) -> Optional[Dict[str, str]]:
        try:
            ...
            # IDENTITY-ATTRIBUTE questions: appearance/morphology, NOT environment.
            # Five slots to mirror the baseline's structure.
            questions = {
                'fur_coat': "Describe the animal's fur, coat, or skin color and pattern.",
                'body_shape': "Describe the animal's body shape and form.",
                'head_face': "Describe the animal's head, face, and ears.",
                'distinctive_markings': "Describe any distinctive markings or features on the animal.",
                'size_build': "Describe the size and build of the animal.",
            }
    ...
    def generate_captions_for_dataset(self, image_paths: List[str], labels: List[int],
        ...
        self.logger.info(f"Generating IDENTITY captions for {len(image_paths)} images...")
        captions_data = {}
        successful_count = 0
        failed_count = 0
        ...
        progress_bar = tqdm(zip(image_paths, labels, species_list),
                            total=len(image_paths), desc="Generating identity captions")
        ...
        self._save_captions(captions_data, output_file)
        self.logger.info(f"Identity caption generation complete! Successful: {successful_count}, Failed: {failed_count}")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Generate IDENTITY-attribute captions")
