...
def run_single_dinov2_stratified_experiment(dataset_path: str, balance_strategy: str,
                                            output_dir: str, run_id: int, seed: int,
                                            experiment_logger) -> Dict[str, Any]:
    
    
    experiment_logger.info(f"\n🦖 Running DINOv2 Stratified Experiment {run_id} (seed={seed})")
    ...
    try:
        ...
        experiment_logger.info(f"✅ DINOv2 stratified experiment {run_id} completed successfully")
...
def run_dinov2_stratified_experiment(dataset_path: str, balance_strategy: str, 
    ...
    log_file = os.path.join(output_dir, 'dinov2_stratified_36runs.log')
    logger = setup_logging("INFO", log_file)
    
    logger.info("="*100)
    logger.info("DINOv2 STRATIFIED EXPERIMENT - 36 RUNS")
    logger.info("="*100)
    logger.info(f"Goal: Test DINOv2 WITHOUT distribution shift")
    ...
    # Run experiments
    experiment_results = {
        'experiment_type': 'dinov2_stratified_36runs',
        'model': 'DINOv2-base',
        'split_type': 'stratified',
        'num_runs': num_runs,
    ...
    for i in range(num_runs):
        ...
        result = run_single_dinov2_stratified_experiment(
    ...
    if successful_runs >= 3:
        ...
        logger.info(f"\n{'='*100}")
        logger.info("DINOV2 STRATIFIED RESULTS SUMMARY")
        logger.info(f"{'='*100}")
    ...
    results_file = os.path.join(output_dir, 'dinov2_stratified_36runs_results.json')
...
def main():
    parser = argparse.ArgumentParser(description="DINOv2 Stratified 36 Runs Experiment")
    parser.add_argument('--dataset_path', type=str, required=True)
    parser.add_argument('--balance_strategy', type=str, default='1:10')
    parser.add_argument('--output_dir', type=str, default='./dinov2_stratified_36runs')
    parser.add_argument('--num_runs', type=int, default=36)
    
    args = parser.parse_args()
    
    run_dinov2_stratified_experiment(
        args.dataset_path,
