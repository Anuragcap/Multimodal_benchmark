...
def run_single_bioclip_ood_experiment(dataset_path: str, balance_strategy: str,
                                      output_dir: str, run_id: int, seed: int,
                                      experiment_logger, test_species_pair: tuple = None) -> Dict[str, Any]:
    """Run a single BioClip experiment with OOD splits"""
    
    experiment_logger.info(f"\n🧬 Running BioClip OOD Experiment {run_id} (seed={seed})")
    ...
    try:
        ...
        experiment_logger.info(f"✅ BioClip OOD experiment {run_id} completed successfully")
...
def run_bioclip_ood_all_combinations(dataset_path: str, balance_strategy: str, output_dir: str):
    """Run BioClip on all 36 OOD species combinations"""
    ...
    log_file = os.path.join(output_dir, 'bioclip_ood_all_combos.log')
    logger = setup_logging("INFO", log_file)
    
    logger.info("="*100)
    logger.info("BIOCLIP OOD EXPERIMENT - ALL 36 SPECIES COMBINATIONS")
    logger.info("="*100)
    ...
    # Run experiments
    experiment_results = {
        'experiment_type': 'bioclip_ood_all_combinations',
    ...
    for run_idx, test_species_pair in enumerate(all_test_combinations):
        ...
        result = run_single_bioclip_ood_experiment(
    ...
    if successful_runs >= 3:
        ...
        logger.info(f"\n{'='*100}")
        logger.info("BIOCLIP OOD RESULTS SUMMARY")
        logger.info(f"{'='*100}")
    ...
    results_file = os.path.join(output_dir, 'bioclip_ood_all_combos_results.json')
...
def main():
    parser = argparse.ArgumentParser(description="BioClip OOD All Combinations Experiment")
    parser.add_argument('--dataset_path', type=str, required=True)
    parser.add_argument('--balance_strategy', type=str, default='1:10')
    parser.add_argument('--output_dir', type=str, default='./bioclip_ood_all_combos')
    
    args = parser.parse_args()
    
    run_bioclip_ood_all_combinations(
        args.dataset_path,
