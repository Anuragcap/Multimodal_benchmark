...
class TextOnlyCLIP(nn.Module):
    ...
    def __init__(self, device: str = "cuda"):
        ...
        # Load CLIP model (we'll only use text encoder)
        ...
        print(f"CLIP Text feature dimension: {self.text_dim}")
        
        # Classification head (text-only)
        ...
        print(f"Initialized text-only model with feature dim: {self.text_dim}")
    ...
    def forward(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        # Classification
        outputs = self.classifier(text_features)
        return outputs
...
class TextOnlyTrainer:
    ...
    def __init__(self, config, logger):
        ...
        # Initialize model
        self.model = TextOnlyCLIP(config.device).to(self.device)
    ...
    def train(self, train_loader: DataLoader, val_loader: DataLoader) -> Dict[str, Any]:
        """Full training loop"""
        self.logger.info("Starting text-only training...")
        ...
        for epoch in range(self.config.training.num_epochs):
            ...
            if val_loss < self.best_val_loss:
                ...
                # Save best model
                model_path = os.path.join(self.config.data.output_dir, 'models', 'best_text_only.pth')
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Text-Only Wildlife Captivity Detection")
    parser.add_argument("--dataset_path", type=str, required=True, help="Path to dataset")
    parser.add_argument("--captions_file", type=str, required=True, help="Path to captions JSON")
    parser.add_argument("--output_dir", type=str, default="text_only_baseline", help="Output directory")
    ...
    # Setup logging
    log_file = os.path.join(config.data.output_dir, 'logs', 'text_only_baseline.log')
    ...
    logger.info("="*80)
    logger.info("TEXT-ONLY WILDLIFE CAPTIVITY DETECTION")
    logger.info("="*80)
    ...
    logger.info(f"Note: Using ONLY text captions (no images)")
    ...
    try:
        ...
        # Note: We still need transforms for consistency, but images won't be used
        ...
        # Initialize trainer
        trainer = TextOnlyTrainer(config, logger)
        ...
        # Save results
        final_results = {
            'experiment_type': 'text_only_baseline',
            'config': {
        ...
        results_path = os.path.join(config.data.output_dir, 'text_only_baseline_results.json')
        save_results(final_results, results_path)
        
        # Print summary
        logger.info("\n" + "="*80)
        logger.info("TEXT-ONLY BASELINE RESULTS SUMMARY")
        logger.info("="*80)
        ...
        logger.info("Text-only baseline training completed successfully!")
