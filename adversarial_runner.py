...
def run_single_ood_experiment(dataset_path: str, balance_strategy: str,
    ...
    """Run a single OOD experiment with 4-way comparison"""
    ...
    try:
        ...
        if not split_analysis['no_overlap']:
        ...
        experiment_logger.info(f"\n🤖 Training Adversarial Multimodal...")
        ...
        experiment_logger.info(f"  ✓ Adversarial: Acc={am['accuracy']:.4f}, F1={am['f1_score']:.4f}, MCC={am['mcc']:.4f}")
        experiment_logger.info(f"    vs Multimodal: {am['accuracy'] - mm['accuracy']:+.4f}")
        experiment_logger.info(f"    Species Disc: {adversarial_results['species_discrimination_accuracy']:.4f}")

       
        experiment_logger.info(f"\n📈 OOD RUN {run_id} COMPLETE - 4-WAY COMPARISON:")
        if test_species_pair:
            experiment_logger.info(f"  Test Species: {test_species_pair}")
        ...
        experiment_logger.info(f"  Total Improvement:   {am['accuracy'] - to['accuracy']:+.4f} ({(am['accuracy'] - to['accuracy'])/to['accuracy']*100:+.2f}%)")
        ...
        experiment_result = {
            ...
            'species_splits': {
                ...
                'no_overlap': split_analysis['no_overlap']
            },
            'text_only_baseline': {
...
def compute_ood_statistics(results: List[Dict]) -> Dict[str, Any]:
    """Compute statistical analysis across all OOD experiments (4-way)"""
    ...
    # Extract all metrics for 4 models
    ...
    adv_acc = [exp['adversarial_multimodal']['test_metrics']['accuracy'] for exp in results]
    adv_f1 = [exp['adversarial_multimodal']['test_metrics']['f1_score'] for exp in results]
    adv_mcc = [exp['adversarial_multimodal']['test_metrics']['mcc'] for exp in results]

    species_disc = [exp['adversarial_multimodal']['species_discrimination_accuracy'] for exp in results]
    ...
    return {
        ...
        'adversarial_vs_multimodal': {
            'accuracy': analyze_paired_metric(multi_acc, adv_acc, 'accuracy'),
            'f1_score': analyze_paired_metric(multi_f1, adv_f1, 'f1_score'),
            'mcc': analyze_paired_metric(multi_mcc, adv_mcc, 'mcc')
        },
        'adversarial_vs_text_only_total': {
            'accuracy': analyze_paired_metric(text_only_acc, adv_acc, 'accuracy'),
            'f1_score': analyze_paired_metric(text_only_f1, adv_f1, 'f1_score'),
            'mcc': analyze_paired_metric(text_only_mcc, adv_mcc, 'mcc')
        },
        'species_discrimination_analysis': {
            'mean_accuracy': float(np.mean(species_disc)),
            'std_accuracy': float(np.std(species_disc)),
            'min_accuracy': float(np.min(species_disc)),
            'max_accuracy': float(np.max(species_disc)),
            'good_adversarial_training': bool(np.mean(species_disc) < 0.5)
        }
    }
...
def run_ood_generalization_experiment(dataset_path: str, balance_strategy: str,
    ...
    """Run comprehensive OOD generalization experiment with 4-way comparison"""

    experiment_name = f"ood_adversarial_4way_{balance_strategy.replace(':', '_')}"
    ...
    logger.info(f"🧬 OOD GENERALIZATION EXPERIMENT - 4-WAY COMPARISON")
    ...
    # Run experiments
    experiment_results = {
        'experiment_type': 'ood_all_combinations_4way',
    ...
    logger.info(f"\n🔬 Running {num_experiments} OOD experiments (4 models each)...")
    ...
    results_file = os.path.join(output_dir, 'ood_generalization_4way_results.json')
    with open(results_file, 'w') as f:
        json.dump(experiment_results, f, indent=2, default=str)

    # Print summary
    logger.info("\n" + "=" * 100)
    logger.info("OOD 4-WAY COMPARISON RESULTS - ALL 36 COMBINATIONS")
    logger.info("=" * 100)
...
def main():
    parser = argparse.ArgumentParser(description="Run OOD experiments with 4-way comparison")
    parser.add_argument('--dataset_path', type=str, required=True)
    parser.add_argument('--balance_strategy', type=str, default='1:10')
    parser.add_argument('--output_dir', type=str, default='./ood_4way_results')

    args = parser.parse_args()

    print("✅ Running ALL 36 OOD combinations with 4-way comparison")
    print("   This tests every possible 2-species OOD scenario")
    print("   Each combination runs 4 models: Text → Vision → Multi → Adversarial")
