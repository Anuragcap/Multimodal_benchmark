...
class SingleModalRN(nn.Module):
    """Single Modal RN using CLIP ViT-B-16"""
    ...
    def __init__(self, device: str = "cuda"):
        ...
        print(f"CLIP ViT-B-16 feature dimension: {feature_dim}")
        ...
        print(f"Initialized single modal ViT-B-16 with feature dim: {feature_dim}")
    
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass"""
        with torch.no_grad():
            # Get image features from CLIP ViT-B-16
...
class SingleModalTrainer:
    """Trainer for single modal model"""
    ...
    def __init__(self, config, logger):
        ...
        # Initialize model
        self.model = SingleModalRN(config.device).to(self.device)
    ...
    def setup_training(self, train_labels: List[int]):
        ...
        # Setup optimizer - only for classifier parameters (CLIP is frozen)
    ...
    def train(self, train_loader: DataLoader, val_loader: DataLoader) -> Dict[str, Any]:
        """Full training loop"""
        self.logger.info("Starting single modal training...")
        ...
        for epoch in range(self.config.training.num_epochs):
            ...
            if val_loss < self.best_val_loss:
                ...
                # Save best model
                model_path = os.path.join(self.config.data.output_dir, 'models', 'best_single_modal.pth')
...
def main():
    """Main training function for baseline single modal"""
    import argparse
    
    parser = argparse.ArgumentParser(description="Baseline Single Modal Wildlife Captivity Detection")
    parser.add_argument("--dataset_path", type=str, required=True, help="Path to dataset")
    parser.add_argument("--output_dir", type=str, default="single_modal_stratified",help="Output directory")
    ...
    # Setup logging
    log_file = os.path.join(config.data.output_dir, 'logs', 'single_modal_baseline.log')
    ...
    logger.info("="*80)
    logger.info("BASELINE SINGLE MODAL WILDLIFE CAPTIVITY DETECTION")
    logger.info("="*80)
    ...
    try:
        ...
        # Create data splits with randomly stratified splitting
        ...
        logger.info("✅ Stratified splits validated - all species present in all splits")
        ...
        # Initialize trainer
        trainer = SingleModalTrainer(config, logger)
        ...
        # Save results
        final_results = {
            'experiment_type': 'baseline_single_modal',
            'config': {
        ...
        results_path = os.path.join(config.data.output_dir, 'baseline_single_modal_results.json')
        save_results(final_results, results_path)
        
        # Print summary
        logger.info("\n" + "="*80)
        logger.info("BASELINE SINGLE MODAL RESULTS SUMMARY")
        logger.info("="*80)
        ...
        logger.info("Baseline single modal training completed successfully!")
