...
class BioClipModel(nn.Module):
    """BioClip-based wildlife captivity detection model
    
    BioClip is CLIP trained on iNaturalist 2021 - specialized for biological images
    Paper: https://imageomics.github.io/bioclip/
    """
    ...
    def __init__(self, device: str = "cuda"):
        ...
        if not BIOCLIP_AVAILABLE:
            raise ImportError(
                "open_clip is required for BioClip. Install with:\n"
                "pip install open_clip_torch"
            )
        
        print("Loading BioClip model (ViT-B/16 trained on iNaturalist)...")
        
        # Load BioClip - it's a ViT-B/16 CLIP model trained on iNaturalist
        # Model info: https://huggingface.co/imageomics/bioclip
        try:
            self.bioclip_model, _, self.bioclip_preprocess = open_clip.create_model_and_transforms(
                'hf-hub:imageomics/bioclip'
            )
        except Exception as e:
            print(f"Failed to load BioClip from HuggingFace Hub: {e}")
            print("Falling back to standard ViT-B/16...")
            # Fallback to regular CLIP if BioClip not available
            self.bioclip_model, _, self.bioclip_preprocess = open_clip.create_model_and_transforms(
        ...
        # Freeze BioClip model
        for param in self.bioclip_model.parameters():
        ...
        print(f"BioClip feature dimension: {feature_dim}")
        ...
        print(f"Initialized BioClip model with {feature_dim}-dim features")
    
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass"""
        with torch.no_grad():
            # Get image features from BioClip
            image_features = self.bioclip_model.encode_image(x)
            # Normalize features
...
class BioClipTrainer:
    ...
    def __init__(self, config, logger):
        ...
        # Initialize model
        self.model = BioClipModel(device=config.device).to(self.device)
    ...
    def setup_training(self, train_labels: List[int]):
        ...
        # Setup optimizer - only for classifier parameters (BioClip is frozen)
    ...
    def train_epoch(self, train_loader: DataLoader) -> Tuple[float, float]:
        ...
        for batch_idx, batch in enumerate(progress_bar):
            # Handle both tuple format (image, label) and dict format
    ...
    def train(self, train_loader: DataLoader, val_loader: DataLoader) -> Dict[str, Any]:
        """Full training loop"""
        self.logger.info("Starting BioClip training...")
        ...
        for epoch in range(self.config.training.num_epochs):
            ...
            if val_loss < self.best_val_loss:
                ...
                # Save best model
                model_path = os.path.join(self.config.data.output_dir, 'models', 'best_bioclip.pth')
...
def main():
    """Main training function for BioClip baseline"""
    import argparse
    
    parser = argparse.ArgumentParser(
        description="BioClip Baseline Wildlife Captivity Detection",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
BioClip is CLIP trained on iNaturalist 2021 dataset.
It should have better biological/wildlife priors than standard CLIP.

Paper: https://imageomics.github.io/bioclip/
Model: https://huggingface.co/imageomics/bioclip

Example:
  python bioclip_baseline.py \\
      --dataset_path /path/to/dataset \\
      --output_dir ./bioclip_results \\
    ...
    parser.add_argument("--output_dir", type=str, default="bioclip_baseline", help="Output directory")
    ...
    parser.add_argument("--split_type", type=str, default="ood",
                       choices=["ood", "stratified"],
                       help="Data split type: 'ood' (species-aware, no overlap) or 'stratified' (all species in all splits)")
    ...
    # Setup logging
    log_file = os.path.join(config.data.output_dir, 'logs', 'bioclip_baseline.log')
    ...
    logger.info("="*80)
    logger.info("BIOCLIP BASELINE WILDLIFE CAPTIVITY DETECTION")
    logger.info("="*80)
    logger.info(f"BioClip: CLIP trained on iNaturalist 2021")
    logger.info(f"Split Type: {args.split_type.upper()}")
    ...
    try:
        ...
        # Create data splits based on split_type
        if args.split_type == "stratified":
            logger.info("\n🎯 Creating STRATIFIED splits (all species in train/val/test)")
            ...
            logger.info("✅ Stratified splits created - all species present in all splits")
        else:  # ood
            logger.info("\n🧬 Creating OOD splits (species-aware, no overlap)")
            splits = create_data_splits(
                image_paths, labels, species_list,
                config.data.train_ratio, config.data.val_ratio, config.data.test_ratio,
                config.seed, logger
            )
            # Validate splits for OOD
            if not validate_splits_integrity(splits, logger):
                raise ValueError("Data leakage detected in OOD splits!")
            logger.info("✅ OOD splits validated - no species overlap")
        ...
        # Initialize trainer
        trainer = BioClipTrainer(config, logger)
        ...
        plot_path = os.path.join(config.data.output_dir, 'plots', 'bioclip_training_history.png')
        plot_training_history(training_results['history'], plot_path)
        
        # Save results
        final_results = {
            'experiment_type': 'bioclip_baseline',
            'split_type': args.split_type,
            'model_info': {
                'name': 'BioClip',
                'description': 'CLIP trained on iNaturalist 2021',
                'source': 'https://huggingface.co/imageomics/bioclip'
            },
            'config': {
        ...
        results_path = os.path.join(config.data.output_dir, 'bioclip_baseline_results.json')
        save_results(final_results, results_path)
        
        # Print summary
        logger.info("\n" + "="*80)
        logger.info("BIOCLIP BASELINE RESULTS SUMMARY")
        logger.info("="*80)
        ...
        logger.info("BioClip baseline training completed successfully!")
